...
data class Player(val name: String, val tiles: List<Tile> = listOf(), val strategy: Strategy, val board: Board) {
    fun doTurn(): Player {
        val turn = Turn(strategy, board)
        val (moves, board) = turn.play()
        // TDOO: handle result of turn
        return Player(name, listOf() , strategy, board)
    }
}
...
class ThrowToFirstTileStrategy(val board:Board) : Strategy() {
    override fun shouldIContinue(moves: List<Move>): Boolean {
        ...
        if (highestTileWithValueNotBiggerThanX(totalValue, board.tiles).value>=totalValue) return false
        return true
    }
